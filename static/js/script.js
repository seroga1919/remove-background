...
function canvaState(canvas, dete, imageObj, img_name, ind) {
...
function mouseUp(e) {
    ...
    if (cState.isDragged) { // If clicked on free surface & dragged, then only create the rectangle
        ...
        if (cState.img_name.substring(cState.img_name.length-3)=="pdf") {
            ...
            myoperation.detect_table[cState.img_name][page_name].push([cState.mouseX, cState.mouseY, cState.newRect.w, cState.newRect.h]);
        } else {
            try {
                myoperation.detect_table[cState.img_name].push([cState.mouseX, cState.mouseY, cState.newRect.w, cState.newRect.h]);
            } catch {

                let pdf_name = cState.img_name.split('_page_')[0] + ".pdf";
                myoperation.detect_table[pdf_name][cState.img_name].push([cState.mouseX, cState.mouseY, cState.newRect.w, cState.newRect.h]);            
            }
...
function mouseMove(e) {
    ...
    if (cState.draggingNew) {   // creating a pseudo rectangle
        ...
        cState.ctx.fillStyle = "#7E1AF966";
        // cState.ctx.fillStyle = "#CFF2C666";
...
function Dblclick(e) {
    ...
    for (var i = l-1; i >= 0; i--) {
        if (cState.shapes[i].contains(mouse.x, mouse.y)) {
            ...
            if (cState.img_name.substring(cState.img_name.length-3)=="pdf") {
                let page_name = cState.img_name.substring(cState.img_name.length-4,0) + `_page_${cState.ind}.jpg`;
                myoperation.detect_table[cState.img_name][page_name].pop();
            } else {
                try {
                    myoperation.detect_table[cState.img_name].pop();
                } catch {
                    let pdf_name = cState.img_name.split('_page_')[0] + ".pdf";
                    myoperation.detect_table[pdf_name][cState.img_name].pop();
